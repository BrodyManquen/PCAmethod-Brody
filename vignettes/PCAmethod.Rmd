...
### Step 3: Use `ggbiplot` and `ggscreeplot` function to visualize PCA
...
a decreasing order. In other words, a scree plot shows the variance explained by
each component in the PCA, starting with the component that explains the most.
Therefore, we would expect to see PC1 with the highest eigenvalue, followed by
PC2, and so on.
